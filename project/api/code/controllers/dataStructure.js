...
const insertLog = require('./log');
  ...
  router.get("/get/:type/:type1/:type2/:pag_tam/:pag_pag", verifyToken, (req, res) => {  /*/ GET  /*/
    ...
    con.query(query, [ tam, act], (err, result) => {
      if (err) {
        console.error(err);
        // LOG - 500 //
        insertLog(req.user.id, req.user.user, '003-001-500-001', "500", "dataStructure-get", JSON.stringify(req.params),'Error en la base de datos', JSON.stringify(err));
      }

      // LOG - 200 //
      insertLog(req.user.id, req.user.user, '003-001-200-001', "200", "dataStructure-get", JSON.stringify(req.params),'Datos recuperados', JSON.stringify(result));
      res.send(result);
    });
  });
  ...
  router.get("/get_list", verifyToken, (req, res) => { /*/ GET LIST /*/
    ...
    Promise.all([queryDatabase(query), queryDatabase(query_2)])
      .then((results) => {
        ...
        // LOG - 200 //
        insertLog(req.user.id, req.user.user, '003-002-200-001', "200", "dataStructure-get_list", "Sin datos",'Datos recuperados', JSON.stringify(results));
        res.send(responseObj);
      })
      .catch((err) => {
        console.error(err);
        // LOG - 500 //
        insertLog(req.user.id, req.user.user, '003-002-500-001', "500", "dataStructure-get_list", "Sin datos",'Error en la base de datos', JSON.stringify(err));
  ...
  router.get("/duplicate/:description", verifyToken, (req, res) => {  /*/ DUPLICATE  /*/
    ...
    con.query(query, (err, result) => {
      if (err) {
        console.error(err);
        // LOG - 500 //
        insertLog(req.user.id, req.user.user, '003-003-500-001', "500", "dataStructure-duplicate", JSON.stringify(req.params),'Error en la base de datos', JSON.stringify(err));
      ...
      // LOG - 200 //
      insertLog(req.user.id, req.user.user, '003-003-200-001', "200", "dataStructure-duplicate", JSON.stringify(req.params),'Datos duplicados', "Sin datos");
  ...
  router.post("", verifyToken, (req, res) => {  /*/ POST  /*/
    ...
    if (!id_variable_data_structure) {
      // LOG - 400 //
      insertLog(req.user.id, req.user.user, '003-004-400-001', "400", "dataStructure-post", JSON.stringify(req.body),"Faltan parámetros", "Sin datos");
    ...
    con.query(query, [description, configuration,identifier_code, id_variable_data_structure], (err, result) => {
      if (err) {
        // LOG - 500 //
        insertLog(req.user.id, req.user.user, '003-004-500-001', "500", "dataStructure-post", JSON.stringify(req.body),'Error en la base de datos', JSON.stringify(err));
        return res.status(500).json({ error: 'Error en la base de datos' });
      }
      if (result.affectedRows === 1) {
        const insertedId = result.insertId;
        // LOG - 201 //
        insertLog(req.user.id, req.user.user, '003-004-201-001', "201", "dataStructure-post", JSON.stringify(req.body),"Datos guardados", "Sin datos");
        return res.status(201).json({ id: insertedId });
      }

      // LOG - 500 //
      insertLog(req.user.id, req.user.user, '003-004-500-002', "200", "dataStructure-post", JSON.stringify(req.body),"No se pudo insertar el registro", "Sin datos");
  ...
  const id_estructure = req.body.id_estructure === "" ? null : req.body.id_estructure;
    ...
    if ((!id_estructure)) {
      // LOG - 400 //
      insertLog(req.user.id, req.user.user, '003-005-400-001', "400", "dataStructure-update", JSON.stringify(req.body),'Se requiere el ID de la estructura y al menos un campo para actualizar', "Sin datos");
    ...
    con.query(query, values, (err, result) => {
      if (err) {
        // LOG - 500 //
        insertLog(req.user.id, req.user.user, '003-005-500-001', "500", "dataStructure-update", JSON.stringify(req.body),'Error en la base de datos', JSON.stringify(err));
        return res.status(500).json({ error: 'Error en la base de datos' });
      }
      if (result.affectedRows > 0) {
        // LOG - 200 //
        insertLog(req.user.id, req.user.user, '003-005-200-001', "200", "dataStructure-update", JSON.stringify(req.body),'Registro actualizado con éxito', "Sin datos");
        return res.status(200).json({ message: 'Registro actualizado con éxito' });
      }

      // LOG - 404 //
      insertLog(req.user.id, req.user.user, '003-005-404-001', "404", "dataStructure-update", JSON.stringify(req.body),'Registro no encontrado', "Sin datos");
  ...
  router.delete("", verifyToken, (req, res) => {  /*/ DELETE  /*/
    ...
    if (isNaN(id_estructure)) {
      // LOG - 400 //
      insertLog(req.user.id, req.user.user, '003-006-400-001', "400", "dataStructure-delete", JSON.stringify(req.body),'ID no válido', "Sin datos");
      return res.status(400).json({ error: 'ID no válido' });
    }
    con.query("DELETE FROM data_estructure WHERE id_estructure = ?", id_estructure, function (err, result) {
      if (err) {
        // LOG - 500 //
        insertLog(req.user.id, req.user.user, '003-006-500-001', "500", "dataStructure-delete", JSON.stringify(req.body),'Error en la base de datos', JSON.stringify(err));
        return res.status(500).json({ error: 'Error en la base de datos' });
      }
      if (result.affectedRows === 0) {
        // LOG - 404 //
        insertLog(req.user.id, req.user.user, '003-006-404-001', "404", "dataStructure-delete", JSON.stringify(req.body),'Estructura de datos no encontrada', "Sin datos");
        return res.status(404).json({ error: 'Estructura de datos no encontrada' });
      }

      // LOG - 200 //
      insertLog(req.user.id, req.user.user, '003-006-200-001', "200", "dataStructure-delete", JSON.stringify(req.body),'Datos eliminados con éxito', "Sin datos");
