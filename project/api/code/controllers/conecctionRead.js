...
const bcrypt = require('bcrypt');
  ...
  router.get("/get/:type/:type1/:type2/:pag_tam/:pag_pag", verifyToken, (req, res) => {  /*/ GET  /*/
    ...
    if (type === 'search') {
      ...
      values = Array(5).fill(likePattern).concat([type1, type2, tam, act]);
    }
  ...
  // Obtener el ID de los parámetros de la solicitud y convertirlo a un entero
  const id = parseInt(req.params.id);

  // Consulta SQL para seleccionar la conexión de lectura por su ID
  const query = "SELECT id, description, mqttQeue, appID, subscribe, enabled FROM conecction_read WHERE id = ?";

  // Ejecutar la consulta SQL con el ID como parámetro
  con.query(query, [id], (err, result) => {
      ...
      // Si se encuentra la conexión de lectura, devolverla en la respuesta
      if (result.length > 0) {
          // Descifrar el accessKey antes de enviarlo en la respuesta (si es necesario)
          const decryptedResult = result.map(row => ({
              ...row,
              //accessKey: decryptMessage(row.accessKey, secretKey)
          }));
          ...
          // Si no se encuentra la conexión de lectura, devolver un error 404
  ...
  // Encriptación del accessKey antes de insertarlo en la base de datos
  const encryptedMessage = encryptMessage(accessKey, secretKey);

  // Consulta SQL preparada para insertar la nueva conexión de lectura
  ...
  router.put("", verifyToken, (req, res) => {  /*/ UPDATE  /*/
    ...
    // Eliminar la coma extra al final de la consulta y agregar la cláusula WHERE
  ...
  // Consulta para eliminar la conexión de lectura
  ...
  router.post("/secret", verifyToken, (req, res) => {  /*/ SECRET  /*/
      ...
      con.query(query, [req.user.id], (err, result) => {
          ...
          bcrypt.compare(password, result[0].password, (bcryptErr, bcryptResult) => {
              ...
              if (bcryptResult) {
                  // Consulta para obtener el secreto de lectura
