...
const SECRET_KEY = process.env.TOKEN; 
  ...
  router.get("/get/:type/:type1/:type2/:pag_tam/:pag_pag", verifyToken, (req, res) => {  /*/ GET  /*/
    ...
    let query = ``;
    if (type0 === 'search') {
      query += `SELECT *,(SELECT COUNT(*) AS total FROM monitoring) as total FROM monitoring`;
      query += ` ORDER BY ${type1} ${type2}`;
    } else {
      query += `SELECT *,(SELECT COUNT(*) AS total FROM monitoring WHERE email LIKE '%${type0}%' OR password LIKE '%${type0}%') as total FROM monitoring`;
  ...
  router.post("/login", (req, res) => {  // POST LOGIN
    ...
    const query = "SELECT * FROM monitoring WHERE email = ? AND password = ?";
    //console.log(query)
  ...
  router.get("/id/:id", verifyToken, (req, res) => {  /*/ ID  /*/
    ...
    const query = "SELECT * FROM monitoring WHERE id = ?";
  ...
  router.get("/duplicate/:email", verifyToken, (req, res) => {  /*/ DUPLICATE  /*/
    ...
    let query = `SELECT email FROM monitoring`;
  ...
  router.post("", verifyToken, (req, res) => {  /*/ POST  /*/
    ...
    const query = "INSERT INTO monitoring (email, password, change_password) VALUES (?, ?, ?)";
  ...
  router.put("/email", verifyToken, (req, res) => {  // UPDATE EMAIL
    ...
    const query = "UPDATE monitoring SET email = ? WHERE id = ?";
  ...
  let query = "UPDATE monitoring SET";
  const values = [];
  if (email) {
    query += " email=?";
    values.push(email);
  ...
  router.put("/password", verifyToken, (req, res) => {  // UPDATE PASSWORD
    ...
    const query = "SELECT * FROM monitoring WHERE id = ? AND password = ?";
    con.query(query, [id, password], (err, result) => {
      ...
      const updateQuery = "UPDATE monitoring SET password = ? WHERE id = ?";
...
router.delete("", verifyToken, (req, res) => {  /*/ DELETE  /*/
  ...
  con.query("DELETE FROM monitoring WHERE id = ?", id, function (err, result) {
    if (err) {
      return res.status(500).json({ error: 'Error en la base de datos' });
    }
    if (result.affectedRows === 0) {
