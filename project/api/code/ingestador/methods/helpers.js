...
function IsCorrectMessage(payload) {
    ...
    if (payload.uplink_message.decoded_payload == undefined || payload.uplink_message.decoded_payload == null) {
        if (process.env.verbose) {
            if (process.env.verbose) console.log('Message without decode_payload: payload.uplink_message.decoded_payload');
        }
        return false;
    }
    ...
    if (payload.uplink_message.decoded_payload.bytes == undefined || payload.uplink_message.decoded_payload.bytes == null
        ...
        if (process.env.verbose) {
            if (process.env.verbose) console.log('Message error: payload.uplink_message.decoded_payload.bytes');
        }
        return false;
    }
    ...
    if (payload.received_at == undefined || payload.received_at == null) {
        if (process.env.verbose) {
            if (process.env.verbose) console.log('Message error: payload.received_at');
        }
        return false;
    }
    ...
    if (payload.end_device_ids.device_id == undefined || payload.end_device_ids.device_id == null) {
        if (process.env.verbose) {
            if (process.env.verbose) console.log('Message error: end_device_ids.device_id');
        }
        return false;
    }
    ...
    if (payload.end_device_ids.application_ids.application_id == undefined || payload.end_device_ids.application_ids.application_id == null) {
        if (process.env.verbose) {
            if (process.env.verbose) console.log('Message error: payload.end_device_ids.application_ids.application_id');
        }
        return false;
    }
    // Si no hay
